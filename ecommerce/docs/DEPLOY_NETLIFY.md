...
Il comando builda in locale (prisma generate + next build via `[build].command` di `netlify.toml`) e carica asset, blob, funzione server e middleware edge.
...
`DATABASE_URL` (Supabase pooler, ruolo `sessa_app`), `SESSION_SECRET`, `ADMIN_SETUP_TOKEN`, `SITE_URL`.
